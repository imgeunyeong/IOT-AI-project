...
sock = socket.create_connection(('127.0.0.1', 9021))
id = ''
serv_msg = ''
i = 0
k = 0
...
class teacherui(QMainWindow):
    def __init__(self):
        ...
        self.graph_2 = self.fig.add_subplot()
        self.graph_animation = FuncAnimation(self.fig, self.make_graph, interval=1000, blit=False)
    ...
    def pie_graph(self, data):  # self 옆에 i를 적어줘야 돌아감
        ...
        for i in range(len(c)):
            self.newdata.append(c[i])
            print(f'2 : {self.newdata}')

        print(f'뉴데이터{self.newdata}')  # 키워드/맞춘문제/시간/이름
        correct = "맞춘문제: " + str(self.newdata[1])
        correct2 = "전체문제: " + str(self.newdata[2])
        correct3 = "틀린문제: " + str(int(self.newdata[2]) - int(self.newdata[1]))
        correct4 = "학생이름: " + (str(self.newdata[4]))
        correct5 = "풀이시간: " + str(round((int(self.newdata[3]) / 60))) + "분"
        self.statis.append(f'{correct4}\n\n{correct2}\n\n{correct}\n\n{correct3}\n\n{correct5}')

    def make_graph(self, i):
        self.graph_2.clear()  # 반복 될 때마다 clear를 해줘야 그래프가 겹치지 않음
        self.test_1 = int(self.newdata[1])
        print(int(self.newdata[1]))
        self.test_2 = int(self.newdata[2]) - int(self.newdata[1])
        print(int(self.newdata[2]) - int(self.newdata[1]))
        self.graph_2.pie([self.test_1, self.test_2], labels=[' Correct', 'Wrong'], autopct='%.1f%%',
    ...
    def manage(self):  # 통계창
        ...
        self.qThread.sig4.connect(self.pie_graph)
