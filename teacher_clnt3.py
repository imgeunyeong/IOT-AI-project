...
sock=socket.create_connection(('127.0.0.1',9019))
id=''
...
class teacherui(QMainWindow):
    ...
    def chatroom(self):        
        ...
        cThread=threading.Thread(target=self.recv,args=()) #서브스레드 생성 #handler 함수를 쓰레드로 돌림
        cThread.demon=True
        cThread.start()
      
        #while(1):
        print('확인')
        #self.counseling_line.returnPressed.connect(lambda:self.counseling_browser.append(self.counseling_line.text()))
    ...
    def recv(self):
        while(1):
            print('쓰레드 확인')
            chat=sock.recv(1024).decode()    
            print(chat)
            self.counseling_browser.append(chat)
